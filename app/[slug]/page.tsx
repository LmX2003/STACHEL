...
export default async function DetailPage({params}:{params:Promise<{slug:string}>}){const {slug}=await params;const page=pages[slug];if(!page)notFound();const servicePage=["bad","heizung","gas","solar","wasser"].includes(slug);return <><Header/><main className={`detail-page ${page.legal?"legal-page":""}`}>
  <section className={`detail-hero ${page.image?"has-image":"no-image"}`}><div className="detail-copy"><div className="breadcrumb"><Link href="/">Startseite</Link><span>/</span><span>{page.title}</span></div><p className="overline">{page.eyebrow}</p><h1>{page.title}</h1><p className="detail-intro">{page.intro}</p></div>{page.image&&<div className="detail-media"><img src={page.image} alt={page.imageAlt||""} style={{objectPosition:page.imagePosition||"center"}}/><span className="image-caption">{page.imageCaption||"Originalprojekt · Haustechnik Stachel"}</span></div>}</section>
...
function linkify(text:string){if(text.includes("Stadtwerke")&&text.includes("80-4222"))return <>Stadtwerke · Gasgeruch — <a href="tel:+49841800">+49 (841) 80-0</a> oder <a href="tel:+49841804222">+49 (841) 80-4222</a></>;if(text.includes("+49")||text.includes("0841")){const number=text.match(/(?:\+49|0)[0-9 ()/-]+/)?.[0];if(number)return <>{text.slice(0,text.indexOf(number))}<a href={`tel:${number.replace(/\D/g,"").replace(/^49/,"+49")}`}>{number}</a>{text.slice(text.indexOf(number)+number.length)}</>}return text}
